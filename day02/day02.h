...
void day02() {
  ...
  for (auto &line : aoc::getLinesFromFile("day02/input.txt")) {
    ...
    for (auto &subset : aoc::splitStrByDelim(line, "; ")) {
      for (auto &cubePair : aoc::splitStrByDelim(subset, ", ")) {
        auto colorCount = aoc::splitStrByDelim(cubePair, " ");
  ...
  aoc::writeToFile(std::to_string(sumOfGameIDs), "day02/output1.txt");

  std::cout << std::to_string(powerSum) << std::endl;
  aoc::writeToFile(std::to_string(powerSum), "day02/output2.txt");
}